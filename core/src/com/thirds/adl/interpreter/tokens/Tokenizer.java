...
import com.thirds.adl.interpreter.tokens.exception.*;
...
public class Tokenizer {
    ...
    /** Position counter on fileContents. 0 is the start of the file. */
    private int pos = 0;

    /** Line number in fileContents. 1 is the first line. */
    public int line = 1;

    /** Column number in each line. 1 is the first column. */
    public int column = 1;
    ...
    /** Was the last token a \r token (to check for \r\n)? */
    private boolean carriageReturnLastToken = false;

    /** Is the program at the end of the file yet? */
    private boolean eof = false;

    /** The index of the start of the mathematical expression (if the program is currently in a mathematical expression clause) else 0. */
    private int mathExpressionIndex = 0;
    ...
    private Token popNextToken() throws TokenizerException {
        ...
        while (currentChar != Character.MIN_VALUE) {
            ...
            if (Character.isWhitespace(currentChar)) {
                ...
                advance();
                return new Token(TokenType.OPR_PLUS, "+", line, column);
                ...
                if (mathExpressionIndex == 0) {
                    advance();
                    mathExpressionIndex = tokenQueue.size;
                } else throw new NestedMathExpressionException(this);

            } else if (currentChar == '>') {
                if (mathExpressionIndex != 0) {
                    advance();
                    Queue<Token> mathTokens = new Queue<>();
                    int tokenQueueSize = tokenQueue.size;
                    for (int i=mathExpressionIndex; i<tokenQueueSize; i++) {
                        mathTokens.addLast(tokenQueue.removeIndex(mathExpressionIndex));
                    }
                    mathExpressionIndex = 0;
                    return new Token(TokenType.MATH_EXPRESSION, new MathExpression(this, mathTokens), line, column);
                } else throw new UnstartedMathExpressionException(this);
        ...
        eof = true;
        if (mathExpressionIndex != 0) throw new UnfinishedMathExpressionException(this);
